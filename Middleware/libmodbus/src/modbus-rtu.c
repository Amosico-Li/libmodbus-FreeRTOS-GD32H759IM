...
static ssize_t _modbus_rtu_send(modbus_t* ctx, const uint8_t* req, int req_length) {
    return modbus_usart_dma_transmit(req, req_length);
}
...
static ssize_t _modbus_rtu_recv(modbus_t* ctx, uint8_t* rsp, int rsp_length, uint16_t timeout_ms) {

    if(xQueueReceive(Modbus_Recv_Buf, rsp, pdMS_TO_TICKS(timeout_ms)) != pdTRUE) {
        return 0;
    }
    return 1;
}
...
static int _modbus_rtu_pre_check_confirmation(modbus_t* ctx, const uint8_t* req, const uint8_t* rsp,
                                              int rsp_length) {
    ...
    if(req[0] != rsp[0] && req[0] != MODBUS_BROADCAST_ADDRESS) {
        if(ctx->debug) {
            fprintf(
                stderr, "The responding slave %d isn't the requested slave %d\n", rsp[0], req[0]);
        }
        errno = EMBBADSLAVE;
        return -1;
    } else {
...
static int _modbus_rtu_flush(modbus_t* ctx) {
    uint8_t data;
    for(;;){
        if(pdFALSE == xQueueReceive(Modbus_Recv_Buf, &data, 0))
            break;
    }
    return 1;
}
